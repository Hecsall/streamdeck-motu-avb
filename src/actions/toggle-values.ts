...
export class ToggleValues extends SingletonAction<ToggleValuesSettings> {
    ...
    private updateActionState = async (ev: any) => {
        ...
        const onValue = parseFloat(ev.payload.settings.onValue);
    ...
    // override async onWillAppear(ev: WillAppearEvent<ToggleValuesSettings>): Promise<void> {
        // {
        //  "type":"willAppear",
        //  "action":{
    ...
    override async onKeyDown(ev: KeyDownEvent<ToggleValuesSettings>): Promise<void> {
        ...
        const parsedOnValue = parseFloat(onValue);
        const parsedOffValue = parseFloat(offValue);
    ...
    override async onSendToPlugin(ev: SendToPluginEvent<JsonValue, ToggleValuesSettings>): Promise<void> {
